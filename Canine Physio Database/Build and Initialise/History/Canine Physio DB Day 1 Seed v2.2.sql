...
-- ============================================================
-- CANINE PHYSIOTHERAPY - DAY 1 SEED DATA V2.2
-- Purpose:
--   Initial practitioner-side setup aligned to the current mobile content.
--
-- Seeds:
--   1) Two practitioners
--   2) Exercise categories
--   3) Exercise library records based on PhysioContent.json
--   4) Ordered exercise instruction steps
--   5) Terms documents and app content blocks based on AppContent.json
--   6) Session skip reasons
-- ============================================================
...
-- ============================================================
-- 3. TERMS DOCUMENTS
-- Seeded now so app-rendered terms content can link back to the
-- authoritative legal/acceptance documents.
-- ============================================================
INSERT INTO TermsDocument (
    ...
    '1.0',
    'TERMS OF SERVICE',
    'By using Hello Buddy Canine Physiotherapy, you agree to use the app responsibly and follow all exercise guidance provided. This app provides general physiotherapy exercises for dogs and is not a substitute for professional veterinary care. Always consult your veterinarian before starting any new exercise programme. You acknowledge that exercise results may vary and that you are responsible for monitoring your dog''s comfort and safety during all activities.',
    CURRENT_TIMESTAMP,
    TRUE
);
...
INSERT INTO TermsDocument (
    ...
    '1.0',
    'PRIVACY POLICY',
    'We collect and store your email address and exercise progress data to provide our service. Your dog''s exercise history is stored securely and used solely to track progress within the app. We do not sell or share your personal information with third parties. Data may be stored locally on your device and synchronised with our secure servers. You may request deletion of your data at any time by contacting support.',
    CURRENT_TIMESTAMP,
    TRUE
);
...
INSERT INTO TermsDocument (
    ...
    'You agree not to misuse the app or attempt to access it using unauthorized methods. The app should only be used for its intended purpose of canine physiotherapy guidance. You must not share your account credentials with others. Any suspected security issues should be reported immediately. We reserve the right to suspend accounts that violate these terms.',
    CURRENT_TIMESTAMP,
    TRUE
);
...
-- ============================================================
-- 4. EXERCISE LIBRARY
-- Seeded from PhysioContent.json so the admin database and mobile
-- content start from the same exercise catalogue and media keys.
-- ============================================================
...
Do not take your dog''s head beyond point of comfort.',
    ...
    7,
    'baited_back_stretch',
    'baited_back_stretch_video.mp4',
    TRUE,
    'https://cdn.caninephysio.local/exercises/baited_back_stretch.jpg',
    'https://cdn.caninephysio.local/exercises/baited_back_stretch_video.mp4'
FROM ExerciseCategory
WHERE CategoryKey = 'mobility';
...
Raise the first pole on the alternate side to the last pole by the same measure of height.',
    ...
    NULL,
    'raised_poles',
    'raised_poles_video.mp4',
    TRUE,
    'https://cdn.caninephysio.local/exercises/raised_poles.jpg',
    'https://cdn.caninephysio.local/exercises/raised_poles_video.mp4'
FROM ExerciseCategory
WHERE CategoryKey = 'functional_rehab';
...
If necessary, you can use wall support by backing them into a corner and placing a cushion to the side of their hindlimbs to allow for hip support either side of their hindlimbs.',
    ...
    NULL,
    'raised_sit_stands',
    'raised_sit_stands_video.mp4',
    TRUE,
    'https://cdn.caninephysio.local/exercises/raised_sit_stands.jpg',
    'https://cdn.caninephysio.local/exercises/raised_sit_stands_video.mp4'
FROM ExerciseCategory
WHERE CategoryKey = 'strengthening';
...
Then do the same in a forward-backward direction.',
    ...
    'step_up_video.mp4',
    TRUE,
    'https://cdn.caninephysio.local/exercises/step_up.jpg',
    'https://cdn.caninephysio.local/exercises/step_up_video.mp4'
FROM ExerciseCategory
WHERE CategoryKey = 'functional_rehab';
...
This should be performed on a flat, non-slip surface and at an active walk without rushing your dog.',
    ...
    'circles.mp4',
    TRUE,
    'https://cdn.caninephysio.local/exercises/circles.jpg',
    'https://cdn.caninephysio.local/exercises/circles.mp4'
FROM ExerciseCategory
WHERE CategoryKey = 'balance_proprioception';
...
Bring the paw upwards slightly to gain a little flexion from the elbow joint.',
    ...
    'give_paw.mp4',
    TRUE,
    'https://cdn.caninephysio.local/exercises/give_paw.jpg',
    'https://cdn.caninephysio.local/exercises/give_paw.mp4'
FROM ExerciseCategory
WHERE CategoryKey = 'mobility';
...
The goal is to keep them at heel but if your dog struggles with pulling, you may want to consider a harness or halti (but only if this does not impact movement negatively).',
    ...
    'slow_lead_walk.mp4',
    TRUE,
    'https://cdn.caninephysio.local/exercises/slow_lead.jpg',
    'https://cdn.caninephysio.local/exercises/slow_lead_walk.mp4'
FROM ExerciseCategory
WHERE CategoryKey = 'functional_rehab';
...
This is similar to a squat, with them using their hindlimbs to push themselves upwards.',
    ...
    'lay_to_stand.mp4',
    TRUE,
    'https://cdn.caninephysio.local/exercises/lay_to_stand.jpg',
    'https://cdn.caninephysio.local/exercises/lay_to_stand.mp4'
FROM ExerciseCategory
WHERE CategoryKey = 'strengthening';
...
Start with a shallow turn, close to the markers and progress to a deeper turn to encourage spinal bending slowly.',
    ...
    'weaving.mp4',
    TRUE,
    'https://cdn.caninephysio.local/exercises/weaving.jpg',
    'https://cdn.caninephysio.local/exercises/weaving.mp4'
FROM ExerciseCategory
WHERE CategoryKey = 'balance_proprioception';

-- ============================================================
-- 5. EXERCISE INSTRUCTION STEPS
-- ============================================================
INSERT INTO ExerciseInstruction (
    ExerciseId,
    StepNumber,
    InstructionText
)
...
-- ============================================================
-- 6. APP CONTENT BLOCKS
-- Supports generation of AppContent.json from the database.
-- ============================================================
INSERT INTO AppContentBlock (
    ContentGroup,
    ContentKey,
    HeaderText,
    BodyText,
    ...
    'about',
    'ABOUT THIS APP',
    'Physiotherapy at home, with practitioner guidance, can significantly improve the recovery and well-being of your dog.

This app enables practitioners provide exercise programs that can be carried out at home.',
    1,
    TRUE
);
INSERT INTO AppContentBlock (
    ...
    'physiotherapy',
    'PHYSIOTHERAPY',
    'Physiotherapy includes massage, exercises, and environmental modifications that can help manage pain and improve mobility.

However, it''s important to consult with a veterinarian or certified canine physiotherapist before starting any at-home physiotherapy to ensure it''s appropriate and safe for your dog.',
    2,
    TRUE
);
INSERT INTO AppContentBlock (
    ...
    'pain',
    'RECOGNISING PAIN',
    'Key signs of pain include changes in behaviour, decreased activity, altered posture, increased vocalization, and changes in daily habits.

If you are concerned about pain or discomfort, stop immediately and contact your practitioner.',
    3,
    TRUE
);
...
WHERE td.DocumentType = 'terms_of_service'
  AND td.VersionNumber = '1.0';
...
SELECT
    ...
    td.TermsDocumentId
FROM TermsDocument td
WHERE td.DocumentType = 'privacy_policy'
  AND td.VersionNumber = '1.0';
...
SELECT
    ...
    td.TermsDocumentId
FROM TermsDocument td
WHERE td.DocumentType = 'acceptable_use'
  AND td.VersionNumber = '1.0';
...
SELECT
    ...
    ei.StepNumber,
    ei.InstructionText
FROM ExerciseInstruction ei
JOIN Exercise e
  ON e.ExerciseId = ei.ExerciseId
ORDER BY e.ExerciseKey, ei.StepNumber;
...
SELECT
    ...
    SortOrder,
    IsActive
FROM AppContentBlock
ORDER BY ContentGroup, SortOrder, ContentKey;
