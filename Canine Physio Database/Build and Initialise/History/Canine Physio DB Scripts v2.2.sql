...
CREATE TABLE Exercise (
    ...
    ImageAssetKey        VARCHAR(150) NULL,
...
-- Ordered instruction steps aligned to the mobile exercise content.
-- InstructionsText on Exercise remains as an optional denormalised field.
-- ============================================================
CREATE TABLE ExerciseInstruction (
    ...
    CONSTRAINT FK_ExerciseInstruction_Exercise
...
-- ============================================================
-- 23. APP CONTENT BLOCK
-- Supports generation of AppContent.json for informational pages,
-- warning text and app-rendered terms/conditions content.
-- TermsDocument remains the authoritative acceptance/audit record.
-- ============================================================
CREATE TABLE AppContentBlock (
    AppContentBlockId    BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
    ContentGroup         ENUM('information', 'terms_conditions', 'warning') NOT NULL,
    ContentKey           VARCHAR(100) NOT NULL,
    HeaderText           VARCHAR(255) NULL,
    BodyText             LONGTEXT NOT NULL,
    SortOrder            SMALLINT UNSIGNED NOT NULL DEFAULT 1,
    IsActive             BOOLEAN NOT NULL DEFAULT TRUE,
    TermsDocumentId      BIGINT UNSIGNED NULL,
    CreatedDate          DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    UpdatedDate          DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    ...
    CONSTRAINT FK_AppContentBlock_TermsDocument
        FOREIGN KEY (TermsDocumentId) REFERENCES TermsDocument (TermsDocumentId)
        ON DELETE SET NULL
        ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;

CREATE INDEX IX_AppContentBlock_Group_Sort ON AppContentBlock (ContentGroup, SortOrder);
CREATE INDEX IX_AppContentBlock_IsActive ON AppContentBlock (IsActive);
CREATE INDEX IX_AppContentBlock_TermsDocumentId ON AppContentBlock (TermsDocumentId);
