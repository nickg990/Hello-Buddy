...
## 6. First task on DevBox — install and seed local MySQL

Development continues on **DevBox**, not the laptop (the laptop has .NET disabled by corporate policy). The DevBox blocks outbound TCP/3306 to Azure MySQL, so a local MySQL instance is needed for the F5 debug loop. Azure MySQL remains the deployment target; Thursday's Container Apps deployment is the first time the app talks to it. See `MSc Assessment/DecisionLog.md` DEC-005 for the full rationale.

### Step 1 — Install MySQL Server natively on DevBox

```powershell
winget install Oracle.MySQL --accept-source-agreements --accept-package-agreements
```

The installer will prompt for a root password — pick a memorable dev-only password (this is **never used in Azure**, only on localhost). Suggested: `devroot`.
...
If the MySQL service didn't start automatically:
...
### Step 2 — Seed the local database from the same scripts used in Azure

From the workspace root:

```powershell
cd "Canine Physio Database/Build and Initialise"
mysql -u root -p < "Canine Physio DB Scripts v2.3 (fresh).sql"
mysql -u root -p < "Canine Physio DB Day 1 Initialise v2.4.sql"
mysql -u root -p < "Canine Physio DB MSc Assessment Seed v1.sql"
```

Enter the dev root password each time when prompted.

### Step 3 — Verify the seed (must return 10)

```powershell
mysql -u root -p -e "USE canine_physiotherapy; SELECT COUNT(*) AS ExerciseCount FROM SessionExercise se JOIN Session s ON se.SessionId = s.SessionId JOIN Programme p ON s.ProgrammeId = p.ProgrammeId WHERE p.TreatmentCaseId = (SELECT TreatmentCaseId FROM TreatmentCase WHERE CaseTitle = 'Buddy hind limb rehabilitation');"
```

Expected output: `ExerciseCount` = `10`. If yes, the local dev database is at parity with Azure and Day 2 can start.

### Step 4 — Confirm Azure MySQL is also still healthy (optional smoke test)

Open Cloud Shell at https://shell.azure.com and run the same verification query against Azure MySQL — confirms the cloud environment is unchanged since Day 1 handover.

```bash
mysql -h mysql-hellobuddy-prod.mysql.database.azure.com -u netintentions -p --ssl -e "USE canine_physiotherapy; SELECT COUNT(*) FROM SessionExercise;"
```

---

## 7. Connection string convention for the web app

The Day 2 web app reads its connection string from configuration. Both environments share the same code path — only configuration differs.

| Environment            | Configuration source                                                | Connection target                                                                |
| ---------------------- | ------------------------------------------------------------------- | -------------------------------------------------------------------------------- |
| `Development` (DevBox) | `appsettings.Development.json` → `ConnectionStrings:CaninePhysioDb` | `Server=localhost;Port=3306;Database=canine_physiotherapy;Uid=root;Pwd=devroot;` |
| `Production` (Azure)   | Key Vault secret `mysql-connection-string` via Managed Identity     | Azure MySQL Flexible Server in UK West with SSL required                         |

`ASPNETCORE_ENVIRONMENT` defaults to `Development` when running under F5 in VS Code and is set to `Production` by the Container App at deploy time.

---

## 8. First task in the new Copilot chat

Open a fresh Copilot chat with this workspace and paste the following prompt to bootstrap Day 2:

> Pick up where Day 1 left off. Read `HANDOVER.md` and `MSc Assessment/DecisionLog.md` first. Confirm local MySQL is seeded (`HANDOVER.md` section 6 step 3 returns `10`), then start Day 2 from `MSc Assessment/Five-Day Delivery Plan.md`. Resolve the four open decisions in `HANDOVER.md` section 3 before writing any code.

---

_Generated by Copilot at end of Day 1 (Tuesday 27 May 2026). All Day 1 commitments verified before this handover was written. Updated to reflect the DevBox-only development plan (DEC-005)._
